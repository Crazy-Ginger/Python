...
def shit_distance(l1,l2):
    dist = inf
    for i in range(len(l1[0])):
        if l1[0][i] in l2[0] and l1[1][i] in l2[1]:
            for j in range(len(l2[0])):
                if l1[0][i] == l2[0][j] and l1[1][i] == l2[1][j] and i!= 0:
                    currDist = abs(l1[0][i]) + abs(l1[1][i])
                    print ("for consideration", currDist)
                    if currDist < dist:
                        dist = currDist
...
def gridder(l1, l2):
    up1, down1, left1, right1 = sizer(l1)
    up2, down2, left2, right2 = sizer(l2)
    up = max(up1, up2)
    ...
    grid = [["#" for i in range(-left-1, right+3)] for j in range(-down-1, up+3)]

    cord1 = coords(l1)
    cord2 = coords(l2)
    for i in range(len(cord1[0])):
        grid[down+cord1[0][i]][left+cord1[1][i]] = "+"

    for i in range(len(cord2[0])):
        if grid[down+cord2[0][i]][left+cord2[1][i]] == "+":
            grid[down+cord2[0][i]][left+cord2[1][i]] = "X"
        else:
            grid[down+cord2[0][i]][left+cord2[1][i]] = "+"
